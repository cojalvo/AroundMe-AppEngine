 ...
 */
@Api(name = "messageEndpoint", namespace = @ApiNamespace(ownerDomain = "aroundme.com", ownerName = "aroundme.com", packagePath = ""))
...
public class MessageEndpoint {
	 ...
	 */
	private static final String API_KEY = "AIzaSyDC3vAPSF-BJm3CmgWixEzEnATSVKBzTfw";
	...
	public static void doSendViaGcm(String message, String key, User user)
			throws IOException {
		// Trim message if needed.
		if (message.length() > 1000) {
			message = message.substring(0, 1000) + "[...]";
		...
		if (result.getMessageId() != null) {
			...
			String error = result.getErrorCodeName();
			if (error.equals(Constants.ERROR_NOT_REGISTERED)) {
				endpoint.removeDeviceInfo(user.getRegistrationId());
				// TODO Maybe i should delete the user from the system?
				// TODO Since in that case the user remove the application.
				// TODO If so, I have to deal with all groups which contain this
				// user.
			}
		}
	}
